...
public class BfsTest {
    ...
    void bfsNoResultFoundShouldThrowError()  {
        try{
            bfs.search(Initialiser.initializeGraph(), new GraphNode("c2"), new GraphNode("b1"), true);
        } catch (IllegalArgumentException e) {
    ...
    void bfsWithLegthOne() throws IllegalArgumentException {
        List<List<GraphEdges>> search = bfs.search(Initialiser.initializeGraph(), new GraphNode("mail"), new GraphNode("ts"), true);
    ...
    void bfsFromMailToO103OneSolution() throws IllegalArgumentException {
        List<List<GraphEdges>> search = bfs.search(Initialiser.initializeGraph(), mail, o103, false);
        List<List<GraphEdges>> expected = new ArrayList<>();
    ...
    void bfsFromMailToO103MultipleSolution() throws IllegalArgumentException {
        List<List<GraphEdges>> search = bfs.search(Initialiser.initializeGraph(), mail, o103, true);
        List<List<GraphEdges>> expected = new ArrayList<>();
    ...
    void bfsFromMailToO109MultipleSolution() throws IllegalArgumentException {
        List<List<GraphEdges>> search = bfs.search(Initialiser.initializeGraph(), mail, o109, true);
        List<List<GraphEdges>> expected = new ArrayList<>();
